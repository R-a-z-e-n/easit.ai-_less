...
const ChatApp: React.FC<ChatAppProps> = ({ user, onSignOut }) => {
  ...
  const addMessageToConversation = useCallback(async (conversationId: string, message: Message) => {
    ...
    if (isGuest && message.role === 'user') {
        if (!hasApiKey) {
            addToast("Please connect your Gemini API key to use AI features.", 'info');
            handleConnectApiKey();
            return;
        }
        try {
            ...
            if (e.message?.includes("Requested entity was not found")) {
                setHasApiKey(false);
                addToast("API key error. Please re-select your API key.", 'error');
            } else {
  ...
  return (
    <div className="flex h-screen w-full bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200 font-sans overflow-hidden">
      ...
      <main className="flex flex-1 flex-col transition-all duration-300 md:pl-64">
        ...
        />
        {hasApiKey === false && (
          <div className="bg-brand-blue/10 border-b border-brand-blue/20 px-4 py-2 flex items-center justify-between animate-slide-up-fade-in">
            <div className="flex items-center gap-2 text-sm text-brand-blue">
              <span className="font-medium">AI features require a Gemini API key.</span>
              <span className="opacity-70">Connect your own key to start chatting.</span>
            </div>
            <button 
              onClick={handleConnectApiKey}
              className="text-xs bg-brand-blue text-white px-3 py-1 rounded-full font-semibold hover:bg-brand-blue/90 transition-colors"
            >
              Connect Key
            </button>
          </div>
